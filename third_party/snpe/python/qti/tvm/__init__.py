...
class MyLoader(Loader):
    ...
    def load_module(self, fullname):
        ...
        # Force the head tvm change to use the qti tvm
        if fullname.startswith('tvm'):
            fullname = fullname.replace('tvm', 'qti.tvm', 1)
            # Always override tvm.nd due to some funky __init__ logic in tvm
            if fullname == 'qti.tvm.nd':
                fullname = 'qti.tvm.runtime.ndarray'
