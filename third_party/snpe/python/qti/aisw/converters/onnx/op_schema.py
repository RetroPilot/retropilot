...
from qti.aisw.converters.onnx.util import code_to_enum, extract_onnx_type
import copy
...
class OpSchemaDict:
    ...
    def reorder_op_schemas(self, model_op_version):
        ...
        :param model_op_version: type:str The global model opset version
        """
        model_op_version = str(model_op_version)
...
class OpSchemaBase:
        ...
        if len(default_attribute) < 3 or not attr_name:
            return
                ...
                raise ValueError(code_to_message.get_error_message("ERROR_UNSUPPORTED_ATTRIBUTE_VALUE")
                                 (attr_name, src_op.op_type, src_op.input[0], default_attribute[2], value))
    ...
    def check_supported_attributes(self, attribute=None):
        """
        ...
        if isinstance(attribute, list):
            for attr in attribute:
                snpe_attrs_names = [snpe_attrs[0] == attr for snpe_attrs in self.attributes()]
        else:
