...
from qti.aisw.converters.backend import snpe_axis_transformer
...
caffe_layer_axes.add_axis_order('Slice', [AxisAnnotation.ANY] * 2, [AxisAnnotation.ANY] * 2)
...
caffe_layer_axes.add_axis_order('BBoxTransform', [AxisAnnotation.ANY, AxisAnnotation.ANY])
caffe_layer_axes.add_axis_order('BoxWithNMSLimit', [AxisAnnotation.ANY, AxisAnnotation.ANY])
caffe_layer_axes.add_axis_order('RoIAlign', [AxisAnnotation.ANY, AxisAnnotation.ANY])
caffe_layer_axes.add_axis_order('Div', [AxisAnnotation.ANY, AxisAnnotation.ANY])
...
snpe_layer_axes.add_axis_order('Slice', [AxisAnnotation.ANY] * 2, [AxisAnnotation.ANY] * 2)
...
snpe_layer_axes.add_axis_order('BBoxTransform', [AxisAnnotation.ANY, AxisAnnotation.ANY])
snpe_layer_axes.add_axis_order('BoxWithNMSLimit', [AxisAnnotation.ANY, AxisAnnotation.ANY])
snpe_layer_axes.add_axis_order('RoIAlign', [AxisAnnotation.ANY, AxisAnnotation.ANY])
snpe_layer_axes.add_axis_order('Div', [AxisAnnotation.ANY, AxisAnnotation.ANY])
...
class BufferProxy(object):
    ...
    def _gen_alias(self, buf, layername):
        ...
        self._logger.debug(code_to_message.get_debugging_message('DEBUG_CAFFE2_BUFFER_ALIAS_GEN')(buf, alias))
        return alias
...
class Caffe2SnapDnnConverter(object):
    ...
    def __init__(self):
        ...
        self._axis_skip_layers = ['data', 'Dropout', 'ConstantFill']
    ...
    def register_ops(self):
        ...
        self.op_registry_["ConstantFill"] = self.add_constant_fill_layer
        self.op_registry_["Conv"] = self.add_conv_layer
        self.op_registry_["ConvTranspose"] = self.add_deconv_layer
        self.op_registry_["Div"] = self.add_elementwise_binary_div_layer
    ...
    def convert(self,
        ...
        self.copyright_str = converter_utils.get_string_from_txtfile(copyright_file)
...
#                code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_PROCESS_OUT_DIMS')(self.get_layer_name(op), op.type, str(self.get_output_dims(op))))
    ...
    def do_post_layer_work(self, op):
        ...
        src_output_names = target_output_names = self.get_output_names(op)
        output_dims = []
        try:
            output_dims = self.get_output_dims(op)
        except Exception as e:
        ...
        # Only save the first output buffer's axis order
        self.save_axis_order(target_output_names[0])
    ...
    def add_box_with_nms_limit_layer(self, op):
        ...
        soft_nms_method = str(args.get('soft_nms_method', 'linear').decode())
    ...
    def add_constant_fill_layer(self, op):
        layer_name = self.get_layer_name(op)
        self.logger.debug(
            code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_LAYER')(str(op.type), layer_name))

        supported_args = ['value', 'dtype', 'shape', 'extra_shape', 'input_as_shape']
        args = self.get_args(op.arg)
        self.check_args(args, supported_args)
        shape = [shape for shape in args.get('shape')]
        value = numpy.full(shape=shape, fill_value=args.get('value',0.0), dtype=numpy.float32)
        return self.model.add_const_layer( name=layer_name,
                                           shape=shape,
                                           value=value,
                                           quantizable=True)
    ...
    def add_elementwise_binary_div_layer(self, op):
        layer_name = self.get_layer_name(op)
        self.logger.debug(
            code_to_message.get_debugging_message('DEBUG_CAFFE2_CONVERT_LAYER')(str(op.type), layer_name))

        supported_args = ['broadcast', 'axis']
        args = self.get_args(op.arg)
        self.check_args(args, supported_args)
        input_names = self.get_input_names(op)
        output_name = self.get_output_name(op)
        return self.model.add_elementwise_binary_div_layer( layer_name,
                                                            input_names,
                                                            output_name)
    ...
    def add_slice_op(self, op):
        ...
        args = self.get_args(op.arg)
        if not all (k in list(args.keys()) for k in ("starts", "ends")):
        ...
        if len(start) != 2 and len(start) != 3 and len(start) != 4:
            raise ValueError(code_to_message.get_error_message('ERROR_CAFFE2_SLICE_ONLY_SUPPORTED_FOR_2_3AND4_DIMS_DATA'))
        ...
        if len(input_dim) == 4:
            ...
            output_dim[0], offsets[0] = get_dim_off(input_dim[0], start[0], end[0])
            output_dim[1], offsets[1] = get_dim_off(input_dim[1], start[1], end[1])
