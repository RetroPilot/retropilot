...
[--tflite TFLITE_DIRECTORY] [-p PYTORCH_DIRECTORY]
...
optional arguments:
 ...
 --tflite TFLITE DIRECTORY     Specifies TFLITE Directory
 -p PYTORCH_DIRECTORY          Specifies PYTORCH Directory
...
function _setup_pytorch()
{
  ...
  # setup an environment variable called $PYTORCH_HOME
  export PYTORCH_HOME=$PYTORCHDIR
  echo "[INFO] Setting PYTORCH_HOME="$PYTORCHDIR
  export PYTHONPATH=$PYTORCH_HOME/dependencies/python:$PYTHONPATH

  return 0
}

function _setup_tflite()
{
  if ! _is_valid_directory $1; then
    return 1
  fi
  ...
  # setup an environment variable called $TFLITE_HOME
  export TFLITE_HOME=$TFLITEDIR
  echo "[INFO] Setting TFLITE="$TFLITEDIR
...
function _cleanup()
{
  ...
  unset -f _setup_tflite
  unset -f _setup_pytorch
  unset -f _check_ndk
...
_setup_snpe
# parse arguments
while getopts "h?c:f:o:t:p:-:" opt; do
  case $opt in
    h  ) _usage; return 0 ;;
    ...
    p  ) _setup_pytorch $OPTARG || return 1 ;;
    -  ) case $OPTARG in
            tflite)
                  tflite_root="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                  _setup_tflite $tflite_root || return 1 ;;
            \? ) return 1 ;;
            esac;;
    \? ) return 1 ;;
  esac
